...
def ensure_assignment_indexes() -> None:
	_add_index("CRM Lead Assignment Queue", ["status", "next_retry_at"], "idx_claq_status_retry")
	_add_index("CRM Lead Assignment Queue", ["lead", "status"], "idx_claq_lead_status")
	_add_index("CRM Lead Assignment Queue", ["priority", "status", "creation"], "idx_claq_priority_status")
	_add_index("CRM Lead Assignment Agent State", ["team", "active", "load_score"], "idx_claas_team_load")
	_add_index("CRM Lead Assignment Agent State", ["agent"], "idx_claas_agent")
	_add_index("CRM Lead Assignment Log", ["lead"], "idx_clal_lead")
	_add_index("CRM Lead Assignment Log", ["new_owner", "creation"], "idx_clal_owner_creation")
	_add_index("CRM Lead Assignment Log", ["action", "creation"], "idx_clal_action_creation")
