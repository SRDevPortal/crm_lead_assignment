...
from crm_lead_assignment.engine.context import get_lead_context, get_pipeline
from crm_lead_assignment.engine.service import assign_lead, clear_lead_assignment, reassign_lead
...
def assign_crm_leads(leads, new_owner: str) -> dict:
	...
	if len(names) > int(settings.bulk_inline_limit or 20):
		frappe.enqueue(
			"crm_lead_assignment.jobs.bulk_assign",
...
def clear_crm_leads(leads) -> dict:
	...
	if len(names) > int(settings.bulk_inline_limit or 20):
		frappe.enqueue(
			"crm_lead_assignment.jobs.bulk_clear",
			queue=settings.bulk_queue,
			timeout=1800,
			leads=names,
