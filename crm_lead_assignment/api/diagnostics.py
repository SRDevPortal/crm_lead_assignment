...
from crm_lead_assignment.engine.service import auto_assign_lead, clear_lead_assignment
...
def delete_mock_crm_leads() -> dict:
	...
	for lead in leads:
		...
		frappe.db.delete("CRM Lead Assignment Queue", {"lead": lead})
		frappe.db.delete("CRM Lead Assignment Log", {"lead": lead})
...
def inspect_pankaj_non_matching_payload() -> dict:
	...
	return {
		...
		"assignment_queue_count": frappe.db.count("CRM Lead Assignment Queue", {"lead": lead}),
		"assignment_log_count": frappe.db.count("CRM Lead Assignment Log", {"lead": lead}),
...
def inspect_reported_01084_to_01087() -> dict:
	...
	return {
		...
		"logs": {
			...
			if frappe.db.exists("CRM Lead Assignment Log", log)
			else None
			for log in logs
		},
	}
...
def repair_reported_01084_to_01087_notifications_and_reasons() -> dict:
	from crm_lead_assignment.engine.sync import sync_crm_notification
	...
	for log_name, reason in log_reason_by_name.items():
		if frappe.db.exists("CRM Lead Assignment Log", log_name):
			frappe.db.set_value("CRM Lead Assignment Log", log_name, "reason", reason, update_modified=False)
...
def inspect_crm_lead_assignment_rules() -> dict:
	if not frappe.db.exists("DocType", "CRM Lead Assignment Rule"):
		return {"rules": []}
...
def inspect_mi_agent_states() -> dict:
	...
	return {
		"states": frappe.get_all(
			"CRM Lead Assignment Agent State",
...
def inspect_crm_notification(name: str) -> dict:
	...
	if doc.reference_doctype == "CRM Lead" and doc.reference_name:
		...
		out["assignment_queue_count"] = frappe.db.count("CRM Lead Assignment Queue", {"lead": doc.reference_name})
		out["assignment_log_count"] = frappe.db.count("CRM Lead Assignment Log", {"lead": doc.reference_name})
...
def inspect_assignment_artifacts(log_name: str | None = None, queue_name: str | None = None) -> dict:
	...
	if log_name and frappe.db.exists("CRM Lead Assignment Log", log_name):
		log_doc = frappe.get_doc("CRM Lead Assignment Log", log_name)
		out["log"] = {
			"name": log_doc.name,
			"lead": log_doc.lead,
	...
	if queue_name and frappe.db.exists("CRM Lead Assignment Queue", queue_name):
		queue_doc = frappe.get_doc("CRM Lead Assignment Queue", queue_name)
		out["queue"] = {
			"name": queue_doc.name,
			"lead": queue_doc.lead,
...
def run_non_matching_pipeline_queue_test() -> dict:
	settings_name = "CRM Lead Assignment Settings"
	previous_settings = {}
	lead_name = None
	...
	try:
		...
		queue_count = frappe.db.count("CRM Lead Assignment Queue", {"lead": lead_name})
		log_count = frappe.db.count("CRM Lead Assignment Log", {"lead": lead_name})
		...
		if lead_name and frappe.db.exists("CRM Lead", lead_name):
			...
			frappe.db.delete("CRM Lead Assignment Queue", {"lead": lead_name})
			frappe.db.delete("CRM Lead Assignment Log", {"lead": lead_name})
...
def run_live_insert_assignment_test() -> dict:
	from crm_lead_assignment.jobs import process_assignment_queue_item
	...
	try:
		...
		queue_name = frappe.db.get_value("CRM Lead Assignment Queue", {"lead": lead_name}, "name")
		result = process_assignment_queue_item(queue_name) if queue_name else None
		owner = frappe.db.get_value("CRM Lead", lead_name, "lead_owner")
		queue_status = frappe.db.get_value("CRM Lead Assignment Queue", queue_name, "status") if queue_name else None
...
def run_mock_assignment_test() -> dict:
	...
	try:
		...
		for index, (source_id, expected_owner) in enumerate(RULE_CASES, start=1):
			...
			previous_disable_hooks = getattr(frappe.flags, "crm_lead_assignment_disable_hooks", False)
			frappe.flags.crm_lead_assignment_disable_hooks = True
			try:
				lead.insert(ignore_permissions=True)
				created.append(lead.name)
			finally:
				frappe.flags.crm_lead_assignment_disable_hooks = previous_disable_hooks
		...
		source_id_docfield = frappe.db.exists(
			"DocField",
			{"parent": "CRM Lead Assignment Rule", "fieldname": "source_id"},
		)
		source_id_column = frappe.db.sql(
			...
			where table_schema = database()
			  and table_name = 'tabCRM Lead Assignment Rule'
