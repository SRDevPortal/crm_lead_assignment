...
def upsert_multi_campaign_rule(
	...
	name = frappe.db.exists("CRM Lead Assignment Rule", rule_name)
	if name:
		doc = frappe.get_doc("CRM Lead Assignment Rule", name)
	else:
		doc = frappe.new_doc("CRM Lead Assignment Rule")
...
def upsert_source_id_rule(
	...
	from crm_lead_assignment.engine.counters import ensure_agent_state
...
def process_queued_campaign_leads(campaigns, limit: int = 500) -> dict:
	...
	rows = frappe.db.sql(
		f"""
		select q.name
		from `tabCRM Lead Assignment Queue` q
