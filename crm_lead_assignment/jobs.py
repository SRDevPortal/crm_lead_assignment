...
def retry_failed_queue() -> None:
	frappe.db.sql(
		"""
		update `tabCRM Lead Assignment Queue`
		set status = 'Retry',
			next_retry_at = %s
		where status = 'Failed'
		  and attempts < 5
...
def rebuild_agent_states() -> None:
	from crm_lead_assignment.engine.counters import rebuild_all
...
def bulk_assign(leads: list[str], new_owner: str, reason: str | None = None) -> dict:
	from crm_lead_assignment.engine.service import assign_lead
...
def bulk_clear(leads: list[str], reason: str | None = None) -> dict:
	from crm_lead_assignment.engine.service import clear_lead_assignment
