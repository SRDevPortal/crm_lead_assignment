...
from crm_lead_assignment.integrations.team import get_team_for_user


def ensure_agent_state(agent: str, team: str | None = None) -> str:
	team = team or get_team_for_user(agent)
	name = f"{team or 'No Team'}::{agent}"
	if frappe.db.exists("CRM Lead Assignment Agent State", name):
		return name

	doc = frappe.get_doc(
		{
			"doctype": "CRM Lead Assignment Agent State",
			"agent": agent,
			"team": team,
			"active": 1,
			"weight": 1,
...
def increment_agent(agent: str | None, team: str | None = None, *, reassigned: bool = False) -> None:
	...
	frappe.db.sql(
		f"""
		update `tabCRM Lead Assignment Agent State`
...
def _find_state(agent: str, team: str | None = None) -> str | None:
	...
	if team:
		filters["team"] = team
	name = frappe.db.get_value("CRM Lead Assignment Agent State", filters, "name")
	if name:
		return name
...
def rebuild_all() -> int:
	if not frappe.db.exists("DocType", "CRM Lead Assignment Agent State"):
		return 0
	...
	for row in rows:
		...
		increment_agent(row.lead_owner, team)
		count += 1
	return count
...
def sync_from_teams() -> int:
	...
	rows = frappe.db.sql(
		"""
		select distinct tu.user, tu.parent as team
	...
	count = 0
	for row in rows:
		ensure_agent_state(row.user, row.team)
		count += 1
	return count
